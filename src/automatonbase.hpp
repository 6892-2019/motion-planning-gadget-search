...
public:
	 ...
	 */
	bool run(std::initializer_list<symbol_type> string) const;

	/**
	 * Runs this automaton on the given string.
	 * @returns true iff the machine accepts the given string of symbol indices
	 */
	template<typename... Symbols, typename = std::enable_if_t<(... && std::is_integral_v<Symbols>)>>
	bool run(Symbols... string) const {
		return run({numeric_cast<symbol_type>(string)...});
	}
