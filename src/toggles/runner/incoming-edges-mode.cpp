...
int incoming_edges_mode(std::string_view db_path, const vector<std::string_view>& gadget_spec) {
	...
	lmdb::dbi gadget_hashtable, gadget_index, names_db, connect_edges, close_edges, mirror_edges;
	vector<pair<uint64_t, lmdb::dbi>> combine_edges;
	{
		...
		combine_edges = open_combine_edge_databases(env, txn);
		txn.commit();
	}
